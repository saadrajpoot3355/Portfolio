...
export default function Experience() {
  ...
  return (
    <section id="experience" className="border-b border-white/10 bg-[#080a11] py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        ...
        {/* Timeline */}
        <ol className="mt-12 border-l border-white/10 pl-6 sm:pl-10 space-y-12">
          {experiences.map((exp, index) => (
            <li key={index} className="relative group">
              {/* Dot indicator */}
              <span 
                aria-hidden="true" 
                className="absolute -left-[31px] sm:-left-[47px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-[#080a11] bg-blue-500 group-hover:scale-125 group-hover:bg-white transition-all duration-300 shadow-md shadow-blue-500/50"
              ></span>

              <div className="rounded-2xl border border-white/10 bg-[#0c0e17] p-6 sm:p-8 transition-colors hover:border-white/25">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <h3 className="font-display text-lg font-bold uppercase tracking-tight text-white sm:text-xl flex items-center gap-2">
                    {exp.title} <span className="text-blue-400">—</span> {exp.company}
                  </h3>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-4 text-xs font-medium uppercase tracking-[0.16em] text-slate-400">
