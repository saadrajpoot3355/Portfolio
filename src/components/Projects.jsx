      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 sm:mt-12 space-y-6 sm:space-y-8">
          {projects.slice(0, 3).map((project) => {
            ...
            return (
              <Reveal key={project.id} threshold={0.08}>
                <article
        ...
        <div className="mt-6 sm:mt-8 grid gap-5 sm:gap-6 md:grid-cols-2">
          {projects.slice(3).map((project, idx) => {
            const Icon = project.icon;
            return (
              <Reveal key={project.id} delay={idx % 2 === 1 ? 80 : 0} threshold={0.08}>
                <article
                  className="group relative flex flex-col overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 bg-[#0c0e17] transition-all duration-300 hover:-translate-y-1 hover:border-white/25 p-5 sm:p-7 h-full justify-between"
                >
                  <div>
                    {project.image && (
